...
// if erim mode
# define __CPI_TBL_GET(off)	\
  ({ void ** ptr = (void **) (CPI_TABLE_ADDR + off);   \
    *ptr;					       \
  })

# define __CPI_TBL_SET(off, val)                     \
  do { void ** ptr = (void **) (CPI_TABLE_ADDR + off); \
    *ptr = val;					       \
  } while(0)

# define __CPI_TBL_GET2(off, b)      \
  ({ void ** ptr = (void **) (CPI_TABLE_ADDR + off + b);	\
    (uintptr_t)*ptr;						\
  })

# define __CPI_TBL_SET2(off, b, val)             \
  do { void ** ptr = (void **) (CPI_TABLE_ADDR + off + b);	\
    *ptr = (void*) val;						\
  } while(0)
