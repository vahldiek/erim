...
#define CPI_LOOP_UNROLL_TRESHOLD 2
  ...
  struct CPIInterfaceFunctions {
    ...
    Function *CPISetFn;
    Function *CPISwitchedSetFn;
    Function *CPIAssertFn;

    Function *CPISetBoundsFn;
    Function *CPISwitchedSetBoundsFn;
    ...
    Function *CPISwitchTrustedFn;
    Function *CPISwitchUntrustedFn;
  ...
  class CPI : public ModulePass {
    ...
    void buildMetadataReload(IRBuilder<true, TargetFolder> &IRB, Value *VPtr,
                             Value *EndPtr, BasicBlock *ExitBB, Value *PPt, int switched);

    void buildMetadataReloadLoop(IRBuilder<true, TargetFolder> &IRB,
                                 Value *VPtr, Value *Size, Value *PPt, int switched);
  ...
  public:
    ...
    virtual bool runOnModule(Module &M) {
      ...
      if (ShowStats) {
        errs() << "CPI FPTR Statistics:\n";

        PrintStat(errs(), NumCalls);
        PrintStat(errs(), NumIndirectCalls);

        PrintStat(errs(), NumStores);
        PrintStat(errs(), NumProtectedStores);
        PrintStat(errs(), NumLoads);
        PrintStat(errs(), NumProtectedLoads);
        PrintStat(errs(), NumInitStores);
        PrintStat(errs(), NumProtectedInitStores);

        PrintStat(errs(), NumBoundsChecks);

        PrintStat(errs(), NumMemcpyLikeOps);
        PrintStat(errs(), NumProtectedMemcpyLikeOps);
        PrintStat(errs(), NumAllocFreeOps);
        PrintStat(errs(), NumProtectedAllocFreeOps);
      }
...
static void CreateCPIInterfaceFunctions(DataLayout *DL, Module &M,
  ...
  IF.CPISwitchedSetFn = CheckInterfaceFunction(M.getOrInsertFunction(
      "__llvm__cpi_switched_set", VoidTy, Int8PtrPtrTy, Int8PtrTy, NULL));
  ...
  IF.CPISwitchedSetBoundsFn = CheckInterfaceFunction(M.getOrInsertFunction(
      "__llvm__cpi_switched_set_bounds", VoidTy, Int8PtrPtrTy, Int8PtrTy,
                                        BoundsTy, NULL));
  ...
  IF.CPISwitchTrustedFn = CheckInterfaceFunction(M.getOrInsertFunction(
      "__llvm__cpi_switch_trusted", VoidTy, NULL));

  IF.CPISwitchUntrustedFn = CheckInterfaceFunction(M.getOrInsertFunction(
      "__llvm__cpi_switch_untrusted", VoidTy, NULL));
  ...
  if (isa<PointerType>(VTy)) {
    ...
    if(!switched)
      IRB.CreateCall2(IF.CPISetFn,
            ...
            IRB.CreateCall2(IF.CPISwitchedSetFn,
		      IRB.CreatePointerCast(VPtr, IRB.getInt8PtrTy()->getPointerTo()),
		      IRB.CreatePointerCast(IRB.CreateLoad(VPtr), IRB.getInt8PtrTy()));
      ...
      if (STy->getArrayNumElements() <= CPI_LOOP_UNROLL_TRESHOLD) {
        for (uint64_t i = 0, e = STy->getArrayNumElements(); i != e; ++i) {
          ...
          buildMetadataReloadLoop(IRB, IRB.CreateGEP(VPtr, Idx),
                                  IRB.getInt64(Size), PPt, switched);
      }
    }
    ...
    if (isa<CompositeType>(VecTy->getElementType())) {
      for (uint64_t i = 0, e = VecTy->getNumElements(); i != e; ++i) {
        Value *Idx[2] = { IRB.getInt64(0), IRB.getInt64(i) };
        buildMetadataReload(IRB, IRB.CreateGEP(VPtr, Idx), EndPtr, ExitBB, PPt, switched);
      }
    }
...
void CPI::buildMetadataReloadLoop(
...
#ifdef ERIM
  if(switched == 0)
     IRB.CreateCall(IF.CPISwitchTrustedFn);  
#endif  
  ...
  if(enable_debug)
    errs() << "looped meta reload";
#ifdef ERIM
  buildMetadataReload(IRB, CurPtr, EndPtr8, Exit, PPt, 1);
#else
  buildMetadataReload(IRB, CurPtr, EndPtr8, Exit, PPt, 0);
#endif
...
#ifdef ERIM
  if (switched == 0)
    IRB.CreateCall(IF.CPISwitchUntrustedFn);
#endif
}
...
bool CPI::runOnFunction(Function &F) {
  ...
  /*  if(F.getName().equals("loop_regs_scan") || F.getName().equals("init_propagate_block_info")) {
    //errs() << "looking into functions";
    errs() <<F.getName();
    enable_debug = 1;
    }*/
...
#warning TODO: move checks out of the loops
  ...
  for (inst_iterator It = inst_begin(F), Ie = inst_end(F); It != Ie;) {
      ...
      if (!RealTy && !RealTy2) {
        ...
        if (r1 || r2) {
          ...
          Value *PPt = insertProfilePoint(IRB, CI, N+"-ty-loop",
                                          IRB.getInt64(0));
          buildMetadataReloadLoop(IRB, RealOp, Size, PPt, 0);
	...
	if(enable_debug)
	  errs() << "calloc builds metadata loop";
        buildMetadataReloadLoop(IRB, RealOp, Size, PPt, 0);
        It.getBasicBlockIterator() = IRB.GetInsertBlock();
        It.getInstructionIterator() = IRB.GetInsertPoint();
	...
	if(enable_debug)
      ...
      if (RealTy && shouldProtectValue(CI, true, CPSOnly, NULL, RealTy)) {
        ...
        Value *PPt = insertProfilePoint(IRB, CI,
                                        "calloc-ty-loop", IRB.getInt64(0));
	if(enable_debug)
	  errs() << "malloc triggers meta loop";
        buildMetadataReloadLoop(IRB, RealOp, Size, PPt, 0);
        It.getBasicBlockIterator() = IRB.GetInsertBlock();
        It.getInstructionIterator() = IRB.GetInsertPoint();
...
#warning Should not do it on every realloc!
	...
	if(enable_debug)
	  errs() << "realloc triggers meta loop";
        buildMetadataReloadLoop(IRB, RealOp, Size, PPt, 0);
        It.getBasicBlockIterator() = IRB.GetInsertBlock();
        It.getInstructionIterator() = IRB.GetInsertPoint();
	...
	if(enable_debug)
      ...
      if (Ptr->getType()->isPointerTy() &&
        ...
        buildMetadataReload(IRB, Ptr, NULL, NULL, PPt, 0);
        It.getBasicBlockIterator() = NextBB;
        It.getInstructionIterator() = NextI;
	if(enable_debug)
	  errs() << "sigaction triggers meta loop";
      }
    } else if (!CPSOnly) {
      ...
      if (N == "_ZNSt14basic_ifstreamIcSt11char_traitsIcEEC1EPKcSt13_Ios_Openmode" ||
        ...
        buildMetadataReload(IRB, CI->getArgOperand(0), NULL, NULL, PPt, 0);
        It.getBasicBlockIterator() = IRB.GetInsertBlock();
        It.getInstructionIterator() = IRB.GetInsertPoint();
      }
    }
  ...
  for (Function::arg_iterator It = F.arg_begin(),
    ...
    buildMetadataReload(IRB, A, NULL, NULL, PPt, 0);
  }
...
Function *CPI::createGlobalsReload(Module &M, StringRef N,
...
#ifdef ERIM
  CI = IRB.CreateCall(IF.CPISwitchTrustedFn);
#endif  
  /*
  ...
  for (Module::global_iterator It = M.global_begin(),
    ...
    buildMetadataReload(IRB, GV, NULL, NULL, PPt, 1);
  }
#ifdef ERIM
  IRB.SetInsertPoint(IRB.GetInsertBlock(),IRB.GetInsertBlock()->getTerminator());
  IRB.CreateCall(IF.CPISwitchUntrustedFn);
#endif
