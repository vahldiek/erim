...
void DarwinClang::AddLinkRuntimeLibArgs(const ArgList &Args,
  ...
  // CPI/CPS requires its own runtime libraries
  int CPILevel = getDriver().GetCPILevel(Args);
  ...
  if (CPILevel != 0) {
    AddLinkRuntimeLib(Args, CmdArgs, CPILibName.str().c_str(), true);
  }

  // SafeStack requires its own runtime libraries
  if (getDriver().GetStackProtectorLevel(Args, *this) == 3) {
    AddLinkRuntimeLib(Args, CmdArgs, "libclang_rt.safestack_osx.a");
  }
