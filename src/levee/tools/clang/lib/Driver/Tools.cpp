...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (getToolChain().getDriver().GetCPILevel(Args) &&
    ...
    if (Args.hasFlag(options::OPT_fcpi_at_lto,
                     options::OPT_fcpi_at_compilation, true)) {
      CmdArgs.push_back("-fcpi-at-lto");
    }
  }

  // link bitcode
  int CPILevel = getToolChain().getDriver().GetCPILevel(Args);
  if (CPILevel &&
      !getToolChain().getDriver().IsUsingLTO(Args)) {
  ...
  unsigned StackProtectorLevel =
      getToolChain().getDriver().GetStackProtectorLevel(Args, getToolChain());
...
void freebsd::Link::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // If we are using CPI and LTO, we need to tell the linker about it
  int CPILevel = ToolChain.getDriver().GetCPILevel(Args);
  if (CPILevel &&
      !Args.hasArg(options::OPT_shared) &&
      !Args.hasArg(options::OPT_nostdlib)) {
    SmallString<128> LibCPI(getToolChain().getDriver().ResourceDir);
  ...
  if (ToolChain.getDriver().GetStackProtectorLevel(Args, ToolChain) == 3 
    ...
    SmallString<128> LibCPI(getToolChain().getDriver().ResourceDir);
    llvm::sys::path::append(LibCPI, "lib", "freebsd",
                            (Twine("libclang_rt.safestack-") +
                             getToolChain().getArchName() + ".a"));
    // CmdArgs.push_back(Args.MakeArgString(LibCPI));
    CmdArgs.push_back("-u");
...
void gnutools::Link::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // If we are using CPI/CPS and LTO, we need to tell the linker about it
  int CPILevel = ToolChain.getDriver().GetCPILevel(Args);
  if (CPILevel &&
      !Args.hasArg(options::OPT_shared) &&
      !Args.hasArg(options::OPT_nostdlib)) {
    ...
    // We also want to keep the low part of the address space free
    // for the two-level lookup table NULL table trick
    if (CPILevel > 1)
      CmdArgs.push_back("-Ttext=0x800000");
  }
