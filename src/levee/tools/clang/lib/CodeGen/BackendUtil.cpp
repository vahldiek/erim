...
static void addCPIPass(const PassManagerBuilder &Builder,
                       PassManagerBase &PM) {
  const PassManagerBuilderWrapper &BuilderWrapper =
      static_cast<const PassManagerBuilderWrapper&>(Builder);
  const CodeGenOptions &CGOpts = BuilderWrapper.getCGOpts();
  if (CGOpts.CPIDelayUntilLTO) {
    PM.add(createCPIPreparePass());
  } else {
    if (Builder.OptLevel == 0) {
      // CPI requires alias analysis even at -O0
      if (Builder.LibraryInfo)
          PM.add(new TargetLibraryInfo(*Builder.LibraryInfo));
...
void EmitAssemblyHelper::CreatePasses(TargetMachine *TM) {
  ...
  if (LangOpts.getCPIProtector() != LangOpts.CPIOff) {
    PMBuilder.addExtension(PassManagerBuilder::EP_ModuleOptimizerEarly,
                           addCPIPass);
