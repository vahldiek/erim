...
CodeGenModule::CodeGenModule(ASTContext &C, const CodeGenOptions &CGO,
  ...
  if (SanOpts.Thread ||
      LangOpts.getCPIProtector() == LangOptions::CPI ||
      LangOpts.getCPIProtector() == LangOptions::CPS ||
...
void CodeGenModule::SetLLVMFunctionAttributesForDefinition(const Decl *D,
  ...
  } else if (LangOpts.getStackProtector() == LangOptions::SafeStack) {
    F->addFnAttr(llvm::Attribute::SafeStack);

    // safestack functions require runtime support, make sure we
    // get this support at link time
    HaveSafeStackFunctions = true;
  }

  if (LangOpts.getCPIProtector() == LangOptions::CPS)
    F->addFnAttr("cps");
  else if (LangOpts.getCPIProtector() == LangOptions::CPI)
    F->addFnAttr("cpi");
...
void CodeGenModule::EmitSafeStackGlobals() {
  if (!HaveSafeStackFunctions)
    return;

#if 0
  llvm::GlobalVariable *GV =
      new llvm::GlobalVariable(TheModule, llvm::Type::getInt8PtrTy(VMContext),
                               false, llvm::GlobalValue::ExternalLinkage,
                               0, "__llvm__unsafe_stack_ptr");
  ...
  llvm::Function *F = llvm::Function::Create(
        ...
        "__llvm__safestack_init", &TheModule);
  LLVMUsed.push_back(F);
}
...
void CodeGenModule::EmitStructsTBAAMetadata() {
  ...
  for (RecordLayoutsTy::const_iterator It = RL.begin(),
...

    {
      ...
      llvm::Value *Ops[2] = { llvm::UndefValue::get(It->second), TyTBAA };
      StructTBAA->addOperand(llvm::MDNode::get(getModule().getContext(), Ops));
    }

    if (Ty->isUnionType()) {
      llvm::SmallVector<llvm::Value*, 4> ElOps;
      ...
      for (RecordDecl::field_iterator i = RD->field_begin(),
        ...
        ElOps.push_back(llvm::UndefValue::get(ElTy));
