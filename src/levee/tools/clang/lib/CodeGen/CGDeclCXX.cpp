...
CreateGlobalInitOrDestructFunction(CodeGenModule &CGM,
  ...
  if (CGM.getLangOpts().getStackProtector() == LangOptions::SSPOn) {
    Fn->addFnAttr(llvm::Attribute::StackProtect);
  } else if (CGM.getLangOpts().getStackProtector() == LangOptions::SSPReq) {
    Fn->addFnAttr(llvm::Attribute::StackProtectReq);
  } else if (CGM.getLangOpts().getStackProtector() == LangOptions::SafeStack) {
    Fn->addFnAttr(llvm::Attribute::SafeStack);

    // safestack functions require runtime support, make sure we
    // get this support at link time
    CGM.setHaveSafeStackFunctions();
  }

  if (CGM.getLangOpts().getCPIProtector() == LangOptions::CPI)
    Fn->addFnAttr("cpi");
  else if (CGM.getLangOpts().getCPIProtector() == LangOptions::CPS)
    Fn->addFnAttr("cps");
