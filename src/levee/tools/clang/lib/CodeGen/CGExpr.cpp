...
void CodeGenFunction::EmitUnderlyingValTypeAnnotation(llvm::Value *V,
                                                      const Expr *E) {
  // Attempt to obtain the underlying type of the expression E for the
  // purpose of TBAA. Go through casts and common binary expression types.
  bool HasCast = false;
  for (;;) {
    if (const CastExpr *CE = dyn_cast<CastExpr>(E)) {
      ...
      E = PE->getSubExpr(); // ParenExpr's are transparent
    } else if (const BinaryOperator *BE = dyn_cast<BinaryOperator>(E)) {
      // Handle ((char*) X + offset) expressions
      if (BE->getType()->isPointerType() &&
          BE->getType()->getPointeeType()->isCharType())
        E = BE->getLHS();
      else
        break;
    } else if (const UnaryOperator *UE = dyn_cast<UnaryOperator>(E)) {
      if (UE->getOpcode() == UO_AddrOf) {
        if (UE->getSubExpr()->getType()->isVoidPointerType()) {
          E = UE->getSubExpr();
          HasCast = true; // HACK: emit void* type annotatoin
          break;
        }
      }
      break;
  ...
  if (HasCast) {
    if (llvm::MDNode *MD = CGM.getTBAAInfo(E->getType())) {
      llvm::Value *Ann = EmitAnnotationCall(
        CGM.getIntrinsic(llvm::Intrinsic::var_annotation),
        Builder.CreateBitCast(V, CGM.Int8PtrTy),
        "tbaa.val", E->getLocStart());
      cast<llvm::Instruction>(Ann)->setMetadata("tbaa.val", MD);
    }
  }
}
...
#if 1
  // CPI: provide extended TBAA info for certain function arguments
  // The following is an ugly hack, we'll have to redo it after the deadline
  if (!CurFn->getName().startswith("__inline_mem")) {
    if (llvm::Function *CalleeF = dyn_cast<llvm::Function>(Callee)) {
      StringRef N = CalleeF->getName();
      bool IsBZero = N.equals("bzero");
      bool IsMemSet = !IsBZero && (N.startswith("llvm.memset") ||
          N.startswith("memset") || N.startswith("__memset") ||
          N.startswith("__inline_memset"));
      bool IsMemCpy = !IsBZero && !IsMemSet && (N.startswith("llvm.memcpy") ||
          N.startswith("memcpy") || N.startswith("__memcpy") ||
          N.startswith("__inline_memcpy"));
      bool IsMemMove = !IsBZero && !IsMemSet && !IsMemCpy &&
          (N.startswith("llvm.memmove") ||
          N.startswith("memmove") || N.startswith("__memmove") ||
          N.startswith("__inline_memmove"));

      if (IsBZero || IsMemSet || IsMemCpy || IsMemMove) {
        if (Args.size() > 0 && Args[0].RV.isScalar())
          EmitUnderlyingValTypeAnnotation(Args[0].RV.getScalarVal(), *ArgBeg);
      }

      if (IsMemCpy || IsMemMove) {
        if (Args.size() > 0 && Args[0].RV.isScalar())
          EmitUnderlyingValTypeAnnotation(Args[1].RV.getScalarVal(), *llvm::next(ArgBeg));
      }
    }
  }
#endif
