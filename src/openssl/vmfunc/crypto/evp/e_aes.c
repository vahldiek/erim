...
ERIM_BUILD_BRIDGE1(void*, memsep_malloc, size_t);
ERIM_BUILD_BRIDGE1(void*, memsep_zalloc, size_t);
ERIM_BUILD_BRIDGE2(void*, memsep_realloc, void*, size_t);
ERIM_BUILD_BRIDGE_VOID1(memsep_free, void*);
                          ...
                          const unsigned char *iv, int enc)
{
	 ...
	 */
	dat->ks = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
                              ...
                              const unsigned char *iv, int enc)
{
	 ...
	 * MEMSEP get key allocation
	 */
	gctx->ks = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
	if (gctx->ks == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
                              ...
                              const unsigned char *iv, int enc)
{
	 ...
	 * MEMSEP get key allocation
	 */
	xctx->ks1 = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
	if (xctx->ks1 == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
	 ...
	 * MEMSEP get key allocation
	 */
	xctx->ks2 = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
	if (xctx->ks2 == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
                              ...
                              const unsigned char *iv, int enc)
{
	 ...
	 * MEMSEP get key allocation
	 */
	cctx->ks = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
	if (cctx->ks == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
                              ...
                              const unsigned char *iv, int enc)
{
    ...
    if(key) {
      /*
		 * MEMSEP get key allocation
		 */
		octx->ksenc = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
		if (octx->ksenc == NULL ) {
			EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		 ...
		 * MEMSEP get key allocation
		 */
		octx->ksdec = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
		if (octx->ksdec == NULL ) {
			EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
			return 0;
		}
...
static int aes_cleanup(EVP_CIPHER_CTX * ctx) {
    ...
    if(dat->ks != NULL) {
    	DBG_PRT("freeing %p\n", dat->ks);
    	(void) ERIM_BRIDGE_CALL(memsep_free, dat->ks);
    	dat->ks = NULL;
    }
    ...
    case EVP_CTRL_COPY:
        {
            ...
            if (dat->ks) {
                ...
                dat_out->ks = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
                if(dat_out->ks == NULL) {
                	return 0;
                }
...
static int aes_gcm_cleanup(EVP_CIPHER_CTX *c)
{
    ...
    if (gctx == NULL)
        return 0;
    if(gctx->ks != NULL) {
    	(void) ERIM_BRIDGE_CALL(memsep_free, gctx->ks);
    	gctx->ks = NULL;
    }
    ...
    case EVP_CTRL_COPY:
        {
            ...
            if (gctx->gcm.key) {
                if (gctx->gcm.key != gctx->ks)
                    return 0;
                /* Memsep allocate new key */
                gctx_out->ks = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));;
                if(gctx_out->ks == NULL) {
                	return 0;
                }
    ...
    if (type == EVP_CTRL_COPY) {
        ...
        if (xctx->xts.key1) {
            if (xctx->xts.key1 != xctx->ks1)
                return 0;
			/* Memsep allocate new key */
            xctx_out->ks1 = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
        ...
        if (xctx->xts.key2) {
            if (xctx->xts.key2 != xctx->ks2)
                return 0;
            /* Memsep allocate new key */
            xctx_out->ks2 = (AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
...
static int aes_xts_cleanup(EVP_CIPHER_CTX * ctx) {
	...
	EVP_AES_XTS_CTX * xctx = EVP_C_DATA(EVP_AES_XTS_CTX,ctx);
	if(xctx->ks1 != NULL) {
		(void) ERIM_BRIDGE_CALL(memsep_free, xctx->ks1);
		xctx->ks1 = NULL;
	}
	if(xctx->ks2 != NULL) {
		(void) ERIM_BRIDGE_CALL(memsep_free, xctx->ks2);
		xctx->ks2 = NULL;
	}
    ...
    case EVP_CTRL_COPY:
        {
            ...
            if (cctx->ccm.key) {
				...
				/* Memsep allocate new key */
				cctx_out->ks =
						(AES_KEY*) ERIM_BRIDGE_CALL(memsep_zalloc, sizeof(AES_KEY));
				if (cctx_out->ks == NULL ) {
					return 0;
				}
...
static int aes_ccm_cleanup(EVP_CIPHER_CTX * ctx) {
    ...
    if(cctx->ks != NULL) {
    	...
    	(void) ERIM_BRIDGE_CALL(memsep_free, cctx->ks);
    	cctx->ks = NULL;
    }
...
static int aes_ocb_cleanup(EVP_CIPHER_CTX *c)
{
    ...
    if(octx->ksenc != NULL) {
    	...
    	(void) ERIM_BRIDGE_CALL(memsep_free, octx->ksenc);
        	octx->ksenc = NULL;
        }
    if(octx->ksdec != NULL) {
    	(void) ERIM_BRIDGE_CALL(memsep_free, octx->ksdec);
    	octx->ksdec = NULL;
    }
