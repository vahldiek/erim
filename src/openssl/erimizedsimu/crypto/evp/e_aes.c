...
ERIM_BUILD_BRIDGE1(void*, erim_mallocIsolated, size_t);
ERIM_BUILD_BRIDGE1(void*, erim_zallocIsolated, size_t);
ERIM_BUILD_BRIDGE2(void*, erim_reallocIsolated, void*, size_t);
ERIM_BUILD_BRIDGE_VOID1(erim_freeIsolated, void*);
...
void gcm_ghash_avx(u64 Xi[2], const u128 Htable[16], const u8 *in,
                   size_t len);
#  define AES_GCM_ASM(gctx)       (gctx->ctr==(ctr128_f)ERIM_BRIDGE_FCTPTR(aesni_ctr32_encrypt_blocks) && \
                                 gctx->gcm.ghash==gcm_ghash_avx)
#  define AES_GCM_ASM2(gctx)      (gctx->gcm.block==(block128_f)ERIM_BRIDGE_FCTPTR(aesni_encrypt) && \
                                 gctx->gcm.ghash==gcm_ghash_avx)
                          ...
                          const unsigned char *iv, int enc)
{
	 ...
	 */
	dat->ks = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
	...
	if ((mode == EVP_CIPH_ECB_MODE || mode == EVP_CIPH_CBC_MODE) && !enc) {
		ret = ERIM_BRIDGE_CALL(aesni_set_decrypt_key, key, EVP_CIPHER_CTX_key_length(ctx) * 8,
				dat->ks);
		dat->block = (block128_f) ERIM_BRIDGE_FCTPTR(aesni_decrypt);
		dat->stream.cbc =
		mode == EVP_CIPH_CBC_MODE ? (cbc128_f) aesni_cbc_encrypt : NULL;
	} else {
		ret = ERIM_BRIDGE_CALL(aesni_set_encrypt_key, key, EVP_CIPHER_CTX_key_length(ctx) * 8,
				dat->ks);
		dat->block = (block128_f) ERIM_BRIDGE_FCTPTR(aesni_encrypt);
		if (mode == EVP_CIPH_CBC_MODE)
		dat->stream.cbc = (cbc128_f) aesni_cbc_encrypt;
		else if (mode == EVP_CIPH_CTR_MODE)
		dat->stream.ctr = (ctr128_f) ERIM_BRIDGE_FCTPTR(aesni_ctr32_encrypt_blocks);
		else
		dat->stream.cbc = NULL;
	}
...
{
	(void) ERIM_BRIDGE_CALL(aesni_cbc_encrypt, in, out, len, EVP_C_DATA(EVP_AES_KEY,ctx)->ks,
                            ...
                            const unsigned char *in, size_t len)
{
    ...
    (void) ERIM_BRIDGE_CALL(aesni_ecb_encrypt, in, out, len, EVP_C_DATA(EVP_AES_KEY,ctx)->ks,
    		                      EVP_CIPHER_CTX_encrypting(ctx));
                              ...
                              const unsigned char *iv, int enc)
{
	 ...
	 * MEMSEP get key allocation
	 */
	gctx->ks = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
	if (gctx->ks == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
    ...
    if (key) {
    	(void) ERIM_BRIDGE_CALL(aesni_set_encrypt_key, key, EVP_CIPHER_CTX_key_length(ctx) * 8,
                              gctx->ks);
        CRYPTO_gcm128_init(&gctx->gcm, gctx->ks, (block128_f) ERIM_BRIDGE_FCTPTR(aesni_encrypt));
        gctx->ctr = (ctr128_f) ERIM_BRIDGE_FCTPTR(aesni_ctr32_encrypt_blocks);
        /*
                              ...
                              const unsigned char *iv, int enc)
{
	 ...
	 * MEMSEP get key allocation
	 */
	xctx->ks1 = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
	if (xctx->ks1 == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
	 ...
	 * MEMSEP get key allocation
	 */
	xctx->ks2 = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
	if (xctx->ks2 == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
    ...
    if (key) {
        ...
        if (enc) {
        	(void) ERIM_BRIDGE_CALL(aesni_set_encrypt_key, key, EVP_CIPHER_CTX_key_length(ctx) * 4,
                                  xctx->ks1);
            xctx->xts.block1 = (block128_f) ERIM_BRIDGE_FCTPTR(aesni_encrypt);
            xctx->stream = aesni_xts_encrypt;
        } else {
        	(void) ERIM_BRIDGE_CALL(aesni_set_decrypt_key, key, EVP_CIPHER_CTX_key_length(ctx) * 4,
                                  xctx->ks1);
            xctx->xts.block1 = (block128_f) ERIM_BRIDGE_FCTPTR(aesni_decrypt);
            xctx->stream = aesni_xts_decrypt;
        }

        (void) ERIM_BRIDGE_CALL(aesni_set_encrypt_key, key + EVP_CIPHER_CTX_key_length(ctx) / 2,
                              EVP_CIPHER_CTX_key_length(ctx) * 4,
                              xctx->ks2);
        xctx->xts.block2 = (block128_f) ERIM_BRIDGE_FCTPTR(aesni_encrypt);
                              ...
                              const unsigned char *iv, int enc)
{
	 ...
	 * MEMSEP get key allocation
	 */
	cctx->ks = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
	if (cctx->ks == NULL ) {
		EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		return 0;
	}
    ...
    if (key) {
        ...
        CRYPTO_ccm128_init(&cctx->ccm, cctx->M, cctx->L,
                           cctx->ks, (block128_f) ERIM_BRIDGE_FCTPTR(aesni_encrypt));
        cctx->str = enc ? (ccm128_f) ERIM_BRIDGE_FCTPTR(aesni_ccm64_encrypt_blocks) :
            (ccm128_f) ERIM_BRIDGE_FCTPTR(aesni_ccm64_decrypt_blocks);
        cctx->key_set = 1;
    }
                              ...
                              const unsigned char *iv, int enc)
{
    ...
    if(key) {
      /*
		 * MEMSEP get key allocation
		 */
		octx->ksenc = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
		if (octx->ksenc == NULL ) {
			EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
		 ...
		 * MEMSEP get key allocation
		 */
		octx->ksdec = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
		if (octx->ksdec == NULL ) {
			EVPerr(EVP_F_AESNI_INIT_KEY, ERR_R_MALLOC_FAILURE);
			return 0;
		}
		...
		do {
			...
			(void) ERIM_BRIDGE_CALL(aesni_set_encrypt_key, key, EVP_CIPHER_CTX_key_length(ctx) * 8,
					octx->ksenc);
			(void) ERIM_BRIDGE_CALL(aesni_set_decrypt_key, key, EVP_CIPHER_CTX_key_length(ctx) * 8,
					octx->ksdec);
			if (!CRYPTO_ocb128_init(&octx->ocb, octx->ksenc, octx->ksdec,
							(block128_f) ERIM_BRIDGE_FCTPTR(aesni_encrypt),
							(block128_f) ERIM_BRIDGE_FCTPTR(aesni_decrypt),
							enc ? (ocb128_f) ERIM_BRIDGE_FCTPTR(aesni_ocb_encrypt)
									: (ocb128_f)ERIM_BRIDGE_FCTPTR(aesni_ocb_decrypt)))
			return 0;
		}while (0);
...
static int aes_cleanup(EVP_CIPHER_CTX * ctx) {
    ...
    if(dat->ks != NULL) {
    	DBG_PRT("freeing %p\n", dat->ks);
    	(void) ERIM_BRIDGE_CALL(erim_freeIsolated, dat->ks);
    	dat->ks = NULL;
    }
    ...
    case EVP_CTRL_COPY:
        {
            ...
            if (dat->ks) {
                ...
                dat_out->ks = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
                if(dat_out->ks == NULL) {
                	return 0;
                }
...
static int aes_gcm_cleanup(EVP_CIPHER_CTX *c)
{
    ...
    if (gctx == NULL)
        return 0;
    if(gctx->ks != NULL) {
    	(void) ERIM_BRIDGE_CALL(erim_freeIsolated, gctx->ks);
    	gctx->ks = NULL;
    }
    ...
    case EVP_CTRL_COPY:
        {
            ...
            if (gctx->gcm.key) {
                if (gctx->gcm.key != gctx->ks)
                    return 0;
                /* Memsep allocate new key */
                gctx_out->ks = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));;
                if(gctx_out->ks == NULL) {
                	return 0;
                }
    ...
    if (EVP_CIPHER_CTX_encrypting(ctx)) {
        ...
        if (gctx->ctr) {
            ...
            if (len >= 32 && AES_GCM_ASM(gctx)) {
                ...
                bulk = ERIM_BRIDGE_CALL(aesni_gcm_encrypt, in, out, len,
                                       gctx->gcm.key,
        ...
        if (gctx->ctr) {
            ...
            if (len >= 16 && AES_GCM_ASM(gctx)) {
                ...
                bulk = ERIM_BRIDGE_CALL(aesni_gcm_decrypt,in, out, len,
                                       gctx->gcm.key,
                                       gctx->gcm.Yi.c, gctx->gcm.Xi.u);
                gctx->gcm.len.u[1] += bulk;
            }
        ...
        if (out == NULL) {
            ...
            if (gctx->ctr) {
                ...
                if (len >= 32 && AES_GCM_ASM(gctx)) {
                    ...
                    bulk = ERIM_BRIDGE_CALL(aesni_gcm_encrypt,in + res,
                                           out + res, len - res,
            ...
            if (gctx->ctr) {
                ...
                if (len >= 16 && AES_GCM_ASM(gctx)) {
                    ...
                    bulk = ERIM_BRIDGE_CALL(aesni_gcm_decrypt,in + res,
                                           out + res, len - res,
                                           gctx->gcm.key,
                                           gctx->gcm.Yi.c, gctx->gcm.Xi.u);
                    gctx->gcm.len.u[1] += bulk;
    ...
    if (type == EVP_CTRL_COPY) {
        ...
        if (xctx->xts.key1) {
            if (xctx->xts.key1 != xctx->ks1)
                return 0;
			/* Memsep allocate new key */
            xctx_out->ks1 = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
        ...
        if (xctx->xts.key2) {
            if (xctx->xts.key2 != xctx->ks2)
                return 0;
            /* Memsep allocate new key */
            xctx_out->ks2 = (AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
...
static int aes_xts_cleanup(EVP_CIPHER_CTX * ctx) {
	...
	EVP_AES_XTS_CTX * xctx = EVP_C_DATA(EVP_AES_XTS_CTX,ctx);
	if(xctx->ks1 != NULL) {
		(void) ERIM_BRIDGE_CALL(erim_freeIsolated, xctx->ks1);
		xctx->ks1 = NULL;
	}
	if(xctx->ks2 != NULL) {
		(void) ERIM_BRIDGE_CALL(erim_freeIsolated, xctx->ks2);
		xctx->ks2 = NULL;
	}
    ...
    case EVP_CTRL_COPY:
        {
            ...
            if (cctx->ccm.key) {
				...
				/* Memsep allocate new key */
				cctx_out->ks =
						(AES_KEY*) ERIM_BRIDGE_CALL(erim_zallocIsolated, sizeof(AES_KEY));
				if (cctx_out->ks == NULL ) {
					return 0;
				}
...
static int aes_ccm_cleanup(EVP_CIPHER_CTX * ctx) {
    ...
    if(cctx->ks != NULL) {
    	...
    	(void) ERIM_BRIDGE_CALL(erim_freeIsolated, cctx->ks);
    	cctx->ks = NULL;
    }
...
static int aes_ocb_cleanup(EVP_CIPHER_CTX *c)
{
    ...
    if(octx->ksenc != NULL) {
    	...
    	(void) ERIM_BRIDGE_CALL(erim_freeIsolated, octx->ksenc);
        	octx->ksenc = NULL;
        }
    if(octx->ksdec != NULL) {
    	(void) ERIM_BRIDGE_CALL(erim_freeIsolated, octx->ksdec);
    	octx->ksdec = NULL;
    }
